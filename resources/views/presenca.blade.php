...
<body>
    ...
    <div class="main-content-wrapper"> {{-- O wrapper principal que será centralizado --}}

        {{-- Mensagens de sucesso ou erro geral (Laravel Session) --}}
        @if(session('success'))
        ...
        {{-- Erro de exceção genérico (se algo muito inesperado acontecer no backend) --}}
        @error('general_error')
        ...
        <div class="calendar-container">
            <div class="calendar">
                <div class="month">
                    ...
                    <div class="date">maio 2024</div> {{-- Este texto será atualizado via JS --}}
        ...
        <div class="presence-container">
            <div class="add-presence">
                <a href="{{ route('cliente.index') }}" class="inicio">Página Inicial</a>
                ...
                <form action="{{ route('presenca.entrada') }}" method="POST">
                    ...
                    <input type="datetime-local" name="entrada" required value="{{ old('entrada') }}">
                    @error('entrada')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                ...
                <form action="{{ route('presenca.saida') }}" method="POST">
                    ...
                    <input type="datetime-local" name="saida" required value="{{ old('saida') }}">
                    @error('saida')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                ...
                <div class="presence-list">
                    <ul>
                        {{-- Conteúdo inicial (do Laravel) - será substituído pelo JS --}}
                        @if ($presencas->isEmpty())
                            <p>Nenhuma presença registrada para este dia.🙁</p>
                        @else
                            @foreach($presencas as $presenca)
                                <li>
                                    {{ $presenca->cliente->nome_crianca }} | Entrada: {{ \Carbon\Carbon::parse($presenca->entrada)->format('H:i') }} | Saída: {{ \Carbon\Carbon::parse($presenca->saida ?? '')->format('H:i') }}
                                </li>
                            @endforeach
                        @endif
                    </ul>
